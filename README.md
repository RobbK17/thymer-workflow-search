...
- **Bulk delete (v1.1.8):** The **trash** icon at the **right** of the header bar toggles **batch mode**. Each result row gets a checkbox; click the row or press **Enter** to toggle the highlighted row. A red-accent footer bar appears when at least one note is selected — **Delete** asks for confirmation, then calls **`record.trash()`** on each (Thymer trash), drops them from **`SearchIndex`**, and re-runs the search. **Cancel** or **Esc** exits batch mode and clears the selection. Click the trash icon again to leave batch mode without deleting.
...
- **`has:link` / `-has:link`** and **`has:attachment` / `-has:attachment`** restrict to records whose **outline body** and/or **properties** contain (or must not contain) links or file attachments (see **Links and attachments** below). Evaluated after the body index runs.
...
- **Autocomplete:** After **`#`**, suggests indexed tags; after **`@`** at the end of the query (when People is configured), suggests people — including right after **`mentions:`** (e.g. **`mentions:@`**), after whitespace (e.g. **`foo @`**), or at the start of the box. **`@` is ignored for autocomplete** when a **word character** sits immediately before **`@`** (so **`user@`** is not treated as a person token). After **`:`** (alone or as in `is:`), suggests **`is:completed`**, **`-is:completed`**, **`has:link`**, **`-has:link`**, **`has:attachment`**, **`-has:attachment`**, **`created:`**, **`updated:`**, **`mentions:`**, **scope prefixes** (**`in:record:`**, **`in:col:`**, **`under:line:`**), and **`title:`** / **`body:`**; after **` or `** (lowercase), offers **` OR `**; after **` and `** (lowercase), offers **` AND `** (parser requires capital **`OR`** / **`AND`**). **v1.1.8:** If **`OR`** or **`AND`** is immediately followed by **`:`** with **no** space (e.g. **`foo OR:`**), choosing **`has:link`**, **`has:attachment`**, or **`-has:`** variants inserts a **leading space** before the token so the query does not become **`ORhas:`** / **`ANDhas:`**. **⌃Space** (Ctrl+Space) opens **saved searches**. While suggestions are open, **↑↓** / **Enter** / **Esc** apply to the list (not the result list); see footer hint in the panel.
...
### Links and attachments (`has:`)

Parsed per segment (like **`is:completed`**). **Negative** forms (**`-has:link`**, **`-has:attachment`**) are stripped **before** the positive tokens so they are not confused with **`has:link`** / **`has:attachment`**.

| Pattern | Meaning |
|--------|---------|
| `has:link` | The record has at least one **link** in the **outline** (segment types such as URL / **ref** / **mention**, or line types such as embedded **document**, collection (**`app`**), global plugin, **transclusion**) **or** in **properties** (linked records, or URL-like **text** values). |
| `-has:link` | The record has **no** such links in outline or properties. |
| `has:attachment` | The record has at least one **attachment** line (**`image`**, **`file`**, **`media`**) **or** a **file / image / banner** property with a file value or blob (via **`files()`**, **`file()`**, **`fileBlob()`**). |
| `-has:attachment` | The record has **no** such attachments. |

Until the **body index** has run for a record, **`has:`** filters treat that record as **not matching** the positive form and as **matching** the negative form (same as other body-derived flags).
...
| Enter | Open the selected record in an adjacent panel — or, in **batch delete** mode, toggle the checkbox on the highlighted row. |
| Esc | In **batch delete** mode: exit batch mode and clear the selection (same as **Cancel** in the footer). |
...
- **Bulk delete:** With batch mode on (**trash** icon), rows show a leading checkbox; the rest of the list layout is unchanged. **v1.1.8** places the trash control as the **rightmost** header icon.
...
2. **Body index**: Line items are loaded in batches; body text is appended, **mention/ref segments** update **`SearchIndex._mentionIndex`** (`Map<personGuid, Set<recordGuid>>`), task completion and **`hasLink` / `hasAttachment`** (outline + **property** scan) are updated, and the open search panel re-runs the current query when indexing completes.
...
- **Bulk delete** — Trash icon (rightmost header control) toggles batch mode: checkboxes on each result, footer **Delete** / **Cancel** when at least one row is selected, confirmation dialog, then **`record.trash()`** per note and removal from the local index (toaster on success). **Enter** toggles the checkbox on the highlighted row; **Esc** exits batch mode and clears the selection.
- **Colon autocomplete** — After **`OR`** / **`AND`** with **no** space before **`:`**, choosing **`has:link`**, **`has:attachment`**, or **`-has:`** variants inserts a **leading space** so the token does not attach to the keyword (e.g. **`ORhas:link`**). Other colon operators are unchanged.

### 1.1.7

- **`has:link` / `-has:link`** — Indexed booleans from outline **segments** (e.g. **`link`**, **`linkobj`**, **`url`**, **`ref`**, **`mention`**, **`bookmark`**, **`hyperlink`**) and **line types** (**`ref`**, **`document`**, **`app`**, **`gplugin`**, **`transclusion`**), plus **properties**: **`linkedRecords()`**, URL-like **`texts()`**.
- **`has:attachment` / `-has:attachment`** — From line types **`image`**, **`file`**, **`media`**, plus **`files()`** / **`file()`** / **`fileBlob()`** on properties.
- **Parser / filter** — **`QueryParser`** strips tokens; **`SearchIndex.updateLinkAttachment`**; **`wsComputeLinkAttachment`** during body batch and **`_refreshBodyForRecord`**; **`WS_AC_COLON_OPS`** and empty-state hint updated.
